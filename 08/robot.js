...
Robot.prototype.onStep = function() {
    ...
    if (this.root.position.z >= length || this.root.position.z <= -length) {
        this.root.rotateY(Math.PI / 2);
        this.root.rotateY(angle);

    }
    else if ( this.root.position.x >= length || this.root.position.x <= -length) {
        this.root.rotateY(Math.PI / 2);
        this.root.rotateY(angle);
    ...
    all_robots.forEach(r =>{
        if (!r.root.position.equals(this.root.position) &&
            r.root.position.distanceTo(this.root.position) < 50) {
            this.root.rotateY(Math.PI / 2);
            this.root.rotateY(angle);
...
Robot.prototype.onAnimate = function () {
    ...
    if (this.movement == 'raise left arm') {
        ...
        this.left_upperleg.quaternion.slerp( new THREE.Quaternion( Math.sin(  -Math.PI/8 ),   // x
            0,                   // y
            0,                   // z
            Math.cos(  -Math.PI/8) ), // w
            0.2 );

        // this.right_upperleg.quaternion.slerp( new THREE.Quaternion(- Math.sin(  -Math.PI/4 ),   // x
        //     0,                   // y
        //     0,                   // z
        ...
        this.right_upperleg.quaternion.slerp( new THREE.Quaternion( Math.sin( -Math.PI/8 ),   // x
            0,                   // y
            0,                   // z
            Math.cos( -Math.PI/8 ) ), // w
            0.3 );
        this.left_upperleg.quaternion.slerp( new THREE.Quaternion(0,0,0,1), 0.1);
        // this.left_upperleg.quaternion.slerp( new THREE.Quaternion( -Math.sin( -Math.PI/4 ),   // x
        //     0,                   // y
        //     0,                   // z
        //     -Math.cos( -Math.PI/4 ) ), // w
        //     0.3 );
        ...
        var bones = [this.left_upperleg,this.right_upperleg, this.left_lowerleg,
        this.left_lowerarm, this.left_upperarm, this.right_upperleg,this.right_upperarm,
        this.right_lowerarm];
        bones.forEach(b=>b.quaternion.slerp(new THREE.Quaternion(0, 0, 0, 1), 0.1));
    }
