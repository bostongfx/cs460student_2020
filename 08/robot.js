...
Robot.prototype.onAnimate = function() {
  ...
  if (this.movement == 'raise left arm') {
    ...
    var T = -Math.PI / 8;
    t = this.left_upperleg.quaternion.slerp(new THREE.Quaternion( Math.sin(T/2), 0, 0, Math.cos(T/2)), 0.1);
    this.right_upperleg.quaternion.slerp(new THREE.Quaternion(Math.sin(-T/2), 0, 0, Math.cos(-T/2)), 0.1);
    this.right_upperarm.quaternion.slerp(new THREE.Quaternion( Math.sin(T/2), 0, 0, Math.cos(T/2)), 0.1);
    this.left_upperarm.quaternion.slerp(new THREE.Quaternion(Math.sin(-T/2), 0, 0, Math.cos(-T/2)), 0.1);
    if (t.w < 0.99) {
      this.movement = 'walk2';
    }
    this.onStep();
    ...
    var T = -Math.PI / 8;
    t = this.right_upperleg.quaternion.slerp(new THREE.Quaternion( Math.sin(T/2), 0, 0, Math.cos(T/2)), 0.1);
...
Robot.prototype.onStep = function() {
  ...
  if ( this.root.position.z > 470 || this.root.position.z < -470) {
    this.root.rotateY(180);
  }
  if ( this.root.position.x > 470 || this.root.position.x < -470) {
    this.root.rotateY(180);
  }
  for (var o in robots) {
    o = robots [o];
    if(o.root.position != this.root.position) {
      if(o.root.position.distanceTo(this.root.position) <= 50) {
        this.root.rotateY(180);
      }
    }
  }
