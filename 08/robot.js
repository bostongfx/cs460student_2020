Robot = function(x, y, z) {
  ...
  // head and texture with image
  var geometry = new THREE.BoxBufferGeometry(15, 15, 15);
  var texture = new THREE.TextureLoader().load( 'x.png' );
...
Robot.prototype.onSetup = function() {

  this.root.translateZ(10);
  if ((this.root.position.z >=  480) || 
      (this.root.position.z <= -480) ||
      (this.root.position.x >=  480) ||
      (this.root.position.x <= -480)) {
    this.root.rotateY(Math.PI);
  }
  for( var i = 0; i < robots.length; i++) {
    if (!this.root.position.equals(robots[i].root.position)) {
      if(this.root.position.distanceTo(robots[i].root.position) < 50) {
        this.root.rotateY(Math.PI);
      }
    } 
  }
  cubePosition = new THREE.Vector3 (0, -60, -200)
  if (this.root.position.distanceTo(cubePosition) < 60) {
    this.root.rotateY(Math.PI);
  }
};
...
Robot.prototype.onAnimate = function() {
  ...
  // pudate head position when dance and move
  this.cube_mesh.position.set( this.root.position.x,
                               this.root.position.y,
                               this.root.position.z );
  ...
  if (this.movement == 'raise left arm') {
    ...
    if (typeof this.dancer === 'undefined') {
      ...
      this.dancer = setInterval(function() {
        ...
        this.root.position.x += shakeneck;
