Robot = function(x, y, z) {
  ...
  var geometry = new THREE.BoxBufferGeometry(20, 20, 20); //credit to Xu Huang Lin
  const texture = new THREE.TextureLoader().load( 'sans.png' );
  var material = new THREE.MeshBasicMaterial( { map: texture } );
  this.san_mesh = new THREE.Mesh( geometry, material );
  this.san_mesh.position.set( x, y, z );
...
Robot.prototype.onStep = function() {
	...
	if (this.root.position.x > 500) {
	 ...
	 for (var i in robots) { //Credit to Alvin
        i = robots[i];
...
Robot.prototype.onAnimate = function() {
   this.san_mesh.position.set( //credit to Xu huang Lin
  ...
  if (this.movement == 'raise_left_arm') {
    ...
    this.right_upperleg.quaternion.slerp( new THREE.Quaternion(0,0,0,1), 0.1 );
      ...
      this.left_upperleg.quaternion.slerp( new THREE.Quaternion( Math.sin( T / 2 ),   // x
                                                                  ...
                                                                  Math.cos( T / 2 ) ), // w
                                            0.1 );
	this.right_upperleg.quaternion.slerp( new THREE.Quaternion(0, 0, 0, 1),
                                        0.1 );
	if (this.left_upperleg.quaternion.w < .93) {
      ...
      this.right_upperleg.quaternion.slerp( new THREE.Quaternion( Math.sin( T / 2 ),   // x
                                                                  ...
                                                                  Math.cos( T / 2 ) ), // w
                                            0.1 );
	this.left_upperleg.quaternion.slerp( new THREE.Quaternion(0, 0, 0, 1),
                                        0.1 );
