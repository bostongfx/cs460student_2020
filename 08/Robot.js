...
Robot.prototype.distanceTo = function(other) {
  return Math.sqrt(
    Math.pow((this.root.position.x-other.root.position.x),2) +
    Math.pow((this.root.position.z-other.root.position.z),2) )
};
...
Robot.prototype.walk = function(robots_arr) {
  this.movement = 'walk';
  this.robots_arr = robots_arr;
};
Robot.prototype.onStep = function() {
  if (this.root.position.x > 140 |
    this.root.position.x < -140 |
    this.root.position.z > 140 |
    this.root.position.z < -140) {
      this.root.rotateY(180);
    }
  //Changed this to 2. 10 was too fast.
  this.root.translateZ(2);
};
...
Robot.prototype.onAnimate = function() {
  ...
  // I combined part 1-5 into a single functionality. So no more 'walk2'.
  // It achieves the same thing. Maybe a little more concise. :)
  else if (this.movement == 'walk') {
    for (i=0; i < this.robots_arr.length; i++) {
      if (this != this.robots_arr[i]) {
        if (this.distanceTo(this.robots_arr[i]) < 50) {
          this.root.rotateY(180);
        }
      }
    }
    // check if slerp reached almost the end
    if (this.r_upper_leg.quaternion.w < 0.93) {
      // signal that the kick is done and the leg should move back
      this.movement = 'walk done';
    }
    else {
      if (this.dancing['walk_cycle'] == true) {
        this.r_upper_leg.quaternion.slerp( getQuaternionX(-Math.PI/4), 0.1 );
        this.l_upper_leg.quaternion.slerp( getQuaternionX(Math.PI/4), 0.1 );
      }
      else {
        this.r_upper_leg.quaternion.slerp( getQuaternionX(Math.PI/4), 0.1 );
        this.l_upper_leg.quaternion.slerp( getQuaternionX(-Math.PI/4), 0.1 );
    ...
    if (this.r_upper_leg.quaternion.w > 0.98) {
      // signal that the kick is done and the leg should move back
      this.movement = 'walk';
      this.dancing['walk_cycle'] = !this.dancing['walk_cycle']
    }
    else {
      // reset leg back to identity
      this.r_upper_leg.quaternion.slerp( getQuaternionReset(), 0.1 );
