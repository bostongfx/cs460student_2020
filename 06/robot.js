...
Robot.prototype.onAnimate = function () {
    if (this.movement == 'raise_left_arm') {
        ...
        for (var i = 0; i < skeletons.length; i++) {
            T = Math.PI * (Math.random() * 2); // 180 degrees
            skeletons[i].slerp(new THREE.Quaternion(
                Math.sin(T / 2) ,
                Math.sin(T / 2) ,
                Math.sin(T / 2) ,
                Math.cos(T / 2)), 0.1);
        ...
        for (var i = 0; i < skeletons.length; i++) {
            skeletons[i].slerp(new THREE.Quaternion(0, 0, 0, 1), 0.1);
        }
    }
};
